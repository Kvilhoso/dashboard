...
const MOCK: Log[] = Array.from({ length: 24 }, (_, i) => ({
  id: i + 1,
  event_type: ['trade_opened', 'trade_closed', 'trade_modified', 'error'][
    i % 4
  ],
  symbol: 'XAUUSD',
...
export default function LogsPage() {
  ...
  const avgLat = Math.floor(
    logs.reduce((s, l) => s + l.latency_ms, 0) / (logs.length || 1),
  );
      ...
      >
        ...
        <div className='flex items-end justify-between'>
          ...
          <p className='text-gray-600 text-sm font-mono'>
            latência avg {avgLat}ms · {errors} erros
          </p>
        </div>
      </motion.div>
          ...
          <thead>
            <tr className='border-b border-white/5'>
              {[
                ...
                <th
                  key={h}
                  className='px-5 py-4 text-left text-[10px] font-mono text-gray-600 uppercase tracking-wider'
                >
                  {h}
                </th>
              ))}
