...
const MOCK: Trade[] = Array.from({ length: 16 }, (_, i) => ({
  id: i + 1,
  symbol: ['XAUUSD', 'XAUUSD', 'XAUUSD', 'XAUUSD', 'XAUUSD'][i % 5],
...
function fmt(v: number) {
  const s = new Intl.NumberFormat('en-US', { minimumFractionDigits: 2 }).format(
    Math.abs(v),
  );
  return `${v < 0 ? '-' : '+'}$${s}`;
}
...
export default function TradesPage() {
  ...
  const [sortField, setSortField] = useState<'profit' | 'opened_at'>(
    'opened_at',
  );
  ...
  const filtered = trades
    ...
    .filter(
      t =>
        ...
        t.account_nickname.toLowerCase().includes(search.toLowerCase()),
    )
    .sort((a, b) => {
      const va =
        sortField === 'profit' ? a.profit : new Date(a.opened_at).getTime();
      const vb =
        sortField === 'profit' ? b.profit : new Date(b.opened_at).getTime();
            ...
            <thead>
              <tr className='border-b border-white/5'>
                {[
                  ...
                  { l: 'Tipo', f: null },
                  { l: 'Volume', f: null },
                  { l: 'Abertura', f: 'opened_at' as const },
                  { l: 'P&L', f: 'profit' as const },
                  { l: 'Status', f: null },
                  { l: 'Conta', f: null },
                  { l: 'Latência', f: null },
                ].map(col => (
                  <th
                    ...
                    className={clsx(
                      'px-5 py-4 text-left text-[10px] font-mono text-gray-600 uppercase tracking-wider',
            ...
            <tbody>
              {filtered.map((t, i) => (
                ...
                >
                  ...
                  <td className='px-5 py-4 font-mono text-gray-400 text-sm'>
                    {t.volume}
                  </td>
                  ...
                  <td
                    className={clsx(
                      'px-5 py-4 num text-sm',
                      t.profit >= 0 ? 'text-white' : 'text-gray-500',
                    )}
                  >
                    {fmt(t.profit)}
                  </td>
                  <td className='px-5 py-4'>
                    <span
                      className={clsx(
                        'text-[9px] font-mono border px-2 py-0.5 rounded-full',
                        t.status === 'open'
