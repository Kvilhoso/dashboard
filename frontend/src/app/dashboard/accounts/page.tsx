...
const MOCK: Account[] = [
  {
    ...
    mt5_login: 12345678,
    mt5_server: 'ClearBroker-Live',
    copy_enabled: true,
    lot_multiplier: 1.0,
  },
  {
    id: 2,
    nickname: 'Carteira Conservadora',
    mt5_login: 87654321,
    mt5_server: 'XPInvestimentos-PRD',
    copy_enabled: true,
    lot_multiplier: 0.5,
  },
  {
    id: 3,
    nickname: 'Agressiva Mini',
    mt5_login: 11223344,
    mt5_server: 'BTGPactual-Real',
    copy_enabled: false,
    lot_multiplier: 2.0,
  },
];
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    try {
      const { data } = await api.post('/accounts', {
        ...form,
        mt5_login: parseInt(form.mt5_login),
        lot_multiplier: parseFloat(form.lot_multiplier),
      });
      ...
      >
        <div>
          ...
          <p className='text-gray-500 text-sm mt-2'>
            {accounts.filter(a => a.copy_enabled).length} de {accounts.length}{' '}
            contas ativas
          </p>
        </div>
        <button
            ...
            >
              <div
                className={clsx(
                  ...
                  acc.copy_enabled ? 'bg-white' : 'bg-gray-700',
                )}
              />

              <div className='flex-1 min-w-0'>
                <p className='text-white font-light'>
                  {acc.nickname || `Conta ${acc.mt5_login}`}
                </p>
              ...
              <div className='text-center shrink-0'>
                ...
                <p className='num text-lg font-light text-white'>
                  {acc.lot_multiplier}
                </p>
              </div>
              ...
              <form onSubmit={handleAdd} className='p-6 space-y-4'>
                ...
                <div className='grid grid-cols-2 gap-3'>
                  <div>
                    ...
                    <input
                      ...
                      onChange={e =>
                        setForm(p => ({ ...p, mt5_login: e.target.value }))
                      }
                      className='input-field'
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={e =>
                        setForm(p => ({ ...p, lot_multiplier: e.target.value }))
                      }
                      className='input-field'
                      placeholder='1.0'
                      type='number'
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e =>
                      setForm(p => ({ ...p, mt5_password: e.target.value }))
                    }
                    className='input-field'
                    type='password'
                  ...
                  <p className='text-[10px] text-gray-700 font-mono mt-1.5'>
                    Criptografada com AES-256 antes de armazenar.
                  </p>
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={e =>
                      setForm(p => ({ ...p, mt5_server: e.target.value }))
                    }
                    className='input-field'
      ...
      <AnimatePresence>
        {deleteId !== null && (
          ...
          >
            ...
            >
              <h3 className='font-light text-white text-xl mb-3'>
                Remover conta?
              </h3>
              <p className='text-gray-500 text-sm mb-8'>
                O copy trading será interrompido imediatamente.
              </p>
              <div className='flex gap-3'>
                <button
                  onClick={() => setDeleteId(null)}
                  className='flex-1 py-3 bg-white/5 border border-white/10 rounded-full text-sm text-gray-400 hover:text-white transition'
                >
                  Cancelar
                </button>
                <button
                  onClick={() => handleDelete(deleteId!)}
                  className='flex-1 py-3 bg-white text-black rounded-full text-sm font-medium hover:bg-gray-100 transition'
                >
                  Remover
                </button>
              </div>
