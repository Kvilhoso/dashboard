  ...
  return (
    <div className='min-h-screen bg-black flex flex-col items-center justify-center px-6 py-12 relative overflow-hidden'>
      ...
      <div
        ...
        style={{
          background:
            'radial-gradient(ellipse 55% 70% at 50% 50%, transparent 20%, rgba(0,0,0,0.75) 100%)',
        }}
      />
      ...
      >
        ...
        <div className='px-8 py-8'>
          ...
          <p className='text-sm text-gray-500 mb-8'>
            Comece gratuitamente, escolha seu plano depois
          </p>
          ...
          <form onSubmit={handleSubmit} className='space-y-4'>
                ...
                <div className='mt-2 space-y-1'>
                  {requirements.map(r => (
                    <div key={r.label} className='flex items-center gap-2'>
                      <Check
                        className={`w-3 h-3 ${r.ok ? 'text-white' : 'text-gray-700'}`}
                      />
                      <span
                        className={`text-[10px] font-mono ${r.ok ? 'text-gray-400' : 'text-gray-700'}`}
                      >
                        {r.label}
                      </span>
                    </div>
            ...
            <button
              type='submit'
              disabled={loading}
              className='w-full bg-white text-black py-3.5 rounded-full text-sm font-medium hover:bg-gray-100 disabled:bg-gray-800 disabled:text-gray-600 disabled:cursor-not-allowed transition-all duration-200 mt-2'
            >
              {loading ? (
